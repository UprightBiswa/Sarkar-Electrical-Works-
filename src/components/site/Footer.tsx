...
import { fmtTime, telHref } from "@/lib/utils";
    ...
    <footer className="relative mt-24 overflow-hidden border-t border-white/10 bg-ink-900">
      ...
      <div className="container-x relative grid gap-12 py-16 md:grid-cols-2 lg:grid-cols-4">
        <div>
          ...
          <p className="mt-4 text-sm leading-6 text-slate-400">{s.tagline}. Honest diagnosis, quality parts and careful repair.</p>
          {socials.length > 0 && (
        ...
        <div>
          ...
          <ul className="mt-4 space-y-3 text-sm text-slate-400">
            ...
            <li className="flex gap-3">
              ...
              <span>
                Opens {fmtTime(s.hours[0]?.open ?? "09:00")}
                <br />
                <Link href="/contact" className="text-xs text-slate-500 hover:text-volt-400">
                  See all hours
                </Link>
              </span>
            </li>
      ...
      <div className="border-t border-white/5">
        <div className="container-x flex flex-col items-center justify-between gap-2 py-6 text-xs text-slate-500 sm:flex-row">
          ...
          <p>Repairing Siliguri&apos;s electricals ⚡</p>
        </div>
      </div>
    </footer>
  );
